...
def fill_by_plane(depth_u16, mask, depth_scale, intr, x_map, y_map, pts3d, tau=0.01):
    ...
    # 평면 방정식: n dot (Z*[x_map, y_map, 1]) + d = 0
    # => Z = -d / (n dot [x_map, y_map, 1])
...
def pca_obb_3d(points_xyz):
    ...
    eigvals_desc = vals[order]  # For stability checking
    axes = vecs[:, order]
...
class PCAPoseFilter:
    def __init__(self, ratio_thresh=1.5, keep_last_when_unstable=True):
        """
        ratio_thresh: r1=λ1/λ2, r2=λ2/λ3 둘 다 이 값보다 커야 '안정'으로 판단
    ...
    try:
        while True:
            ...
            for i in range(len(boxes)):
                ...
                # prefer mask to depth backprojection
                ...
                # PCA OBB with eigenvalues
                center3d, axes3, lens3, corners3d, eigvals_desc = pca_obb_3d(pts3d)
                
                # Apply PCA pose filter for stability
                center3d, axes3, lens3, corners3d, is_stable = pca_filter.update(
                    center3d, axes3, lens3, corners3d, eigvals_desc)
                
                # Apply additional pose stabilizer (roll/pitch lock + temporal smoothing)
                ...
                if ok[0]:
                    ...
                    cv2.rectangle(overlay, (cx, y_text-th-6), (cx+tw+6, y_text), color_bg, -1)
